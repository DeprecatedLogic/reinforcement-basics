...
# Assuming you have an Enum or class for Colors. Adjust if needed!
...
def test_player_environment_color_exception():
    ...
    with pytest.raises(ValueError, match="environment"):
        Player(name="Sneaky", color=env_color)
...
def test_model_initial_assignment():
    ...
    # 1. Assert both players are sitting on valid start lines
    assert player1.position in {(2, 2), (2, 3)}
    assert player2.position in {(2, 2), (2, 3)}
    
    # 2. Assert they didn't spawn on top of each other
    assert player1.position != player2.position
    
    # 3. Assert the laps dictionary was initialized correctly
    assert model.laps_completed[player1] == 0
    assert model.laps_completed[player2] == 0
    
    # 4. Assert player attributes were reset/initialized
