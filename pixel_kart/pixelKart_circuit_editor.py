...
from pixel_kart.pixelKart_circuitFrames import CircuitEditorFrame
...
class CircuitEditor(tk.Toplevel):
    ...
    def __init__(self, parent, callback):
        ...
        keys = list(self.all_circuits.keys())
        if not keys:
            keys = ["No circuits saved yet"]
        self.circuit_var.set(keys[0])

        self.circuit_dropdown = tk.OptionMenu(import_frame, self.circuit_var, *keys)
